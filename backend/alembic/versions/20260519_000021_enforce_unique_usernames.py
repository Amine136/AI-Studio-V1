...
def upgrade() -> None:
    # Normalize legacy usernames, then make duplicates unique before adding
    # the database constraint. This keeps staging migration-safe even if test
    # accounts already share a username.
    op.execute(
        """
        UPDATE users
        SET username = LEFT(
