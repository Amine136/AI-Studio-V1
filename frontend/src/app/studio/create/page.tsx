...
async function normalizeUploadImage(file: File, constraints: UploadImageConstraints): Promise<File> {
  ...
  try {
    ...
    if (blob.size >= file.size && file.size <= MAX_PROXY_IMAGE_BYTES && file.type === outputType) {
      return file;
    }
...
export default function Home() {
  ...
  const uploadInputImageFiles = useCallback(async (selectedFiles: File[]) => {
    ...
    const constraints = getUploadConstraints(selectedImageModelRef.current);
    const providerLabel = providerLabelForModelId(selectedImageModelRef.current);
    try {
